...
class Dump {
    ...
    if let arr = enumeratorAtPath?.allObjects as? [String] {
      for item in arr {
              ...
              if output.contains("LC_VERSION_MIN_IPHONEOS") || output.contains("platform 2") {
                if !ignoreIOSOnlyCheck {
                  consoleIO.writeMessage(
                    "This app can't run on Mac M1 ! However, you can decrypt it anyway by adding argument --ignore-ios-check"
                  )
        ...
        if item.hasSuffix(".framework") {
          let frameName =
            item.components(separatedBy: "/").last?.components(separatedBy: ".framework").first
            ?? ""
          if frameName != "" {
        ...
        if item.hasSuffix(".appex") {
          let exName =
            item.components(separatedBy: "/").last?.components(separatedBy: ".appex").first ?? ""
          if exName != "" {
    ...
    for (i, sourcePath) in needDumpFilePaths.enumerated() {
      ...
      Dump.mapFile(path: sourcePath, mutable: false) {
        ...
        if let base = base_raw {
          Dump.mapFile(path: targetPath, mutable: true) {
            dupe_size, dupe_descriptor, dupe_error, dupe_raw in
            if let dupe = dupe_raw {
              if base_size == dupe_size {
                let header = UnsafeMutableRawPointer(mutating: dupe).assumingMemoryBound(
                ...
                guard
                  var curCmd = UnsafeMutablePointer<load_command>(
                ...
                for _: UInt32 in 0..<header.pointee.ncmds {
                  ...
                  if segCmd.pointee.cmd == LC_ENCRYPTION_INFO_64 {
                    let command = UnsafeMutableRawPointer(mutating: segCmd).assumingMemoryBound(
                      to: encryption_info_command_64.self)
                    let result = Dump.dump(
                      descriptor: base_descriptor, dupe: dupe, info: command.pointee)
                    if result.0 {
                  ...
                  curCmd = UnsafeMutableRawPointer(curCmd).advanced(by: Int(curCmd.pointee.cmdsize))
                ...
                munmap(dupe, dupe_size)
                consoleIO.writeMessage(
                  "If the files are not of the same size, then they are not duplicates of each other, which is an error.",
                  to: .error)
              }
            } else {
              munmap(base, base_size)
  ...
  static func dump(
    descriptor: Int32, dupe: UnsafeMutableRawPointer, info: encryption_info_command_64
  ) -> (Bool, String) {
    ...
    let error = mremap_encrypted(
      base!, cryptsize, info.cryptid, UInt32(CPU_TYPE_ARM64), UInt32(CPU_SUBTYPE_ARM64_ALL))
    if error != 0 {
  ...
  static func mapFile(
    path: UnsafePointer<CChar>, mutable: Bool,
    handle: (Int, Int32, String, UnsafeMutableRawPointer?) -> Void
  ) {
    ...
    let base = mmap(
      nil, Int(s.st_size), mutable ? (PROT_READ | PROT_WRITE) : PROT_READ,
